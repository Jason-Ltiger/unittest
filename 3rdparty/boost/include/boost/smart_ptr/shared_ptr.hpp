...
template< class T, class R > struct sp_enable_if_auto_ptr< std::shared_ptr< T >, R >
{
    typedef R type;
}; 
...
// ... except in C++0x, move disables the implicit copy
    ...
    template<class Y>
    explicit shared_ptr( std::shared_ptr<Y> & r ): px(r.get()), pn()
    {
        boost::detail::sp_assert_convertible< Y, T >();
    ...
    template<class Y>
    shared_ptr( std::shared_ptr<Y> && r ): px(r.get()), pn()
    {
        boost::detail::sp_assert_convertible< Y, T >();
    ...
    template<class Y>
    shared_ptr & operator=( std::shared_ptr<Y> & r )
    {
        this_type( r ).swap( *this );
        return *this;
    }
    ...
    shared_ptr & operator=( std::shared_ptr<Y> && r )
    {
        this_type( static_cast< std::shared_ptr<Y> && >( r ) ).swap( *this );
        return *this;
    }
